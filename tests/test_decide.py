...
from showdown.battle_bots.nash_equilibrium.main import (
    get_weighted_choices_from_multiple_score_lookups,
)
...
class TestGetWeightedChoices(unittest.TestCase):
    def setUp(self):
        self.find_nash_equilibrium_patch = mock.patch(
            "showdown.battle_bots.nash_equilibrium.main.find_nash_equilibrium"
        )
    ...
    def test_returns_correct_values_for_score_lookups_with_different_moves_containing_some_overlap(
        self,
    ):
        self.find_nash_mock.side_effect = [
            (["a", "b"], ["c", "d"], [1, 0], [0, 1], None),
