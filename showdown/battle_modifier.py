...
def find_pokemon_in_reserves(pkmn_name, reserves):
    for reserve_pkmn in reserves:
        if (
            pkmn_name.startswith(reserve_pkmn.name)
            or reserve_pkmn.name.startswith(pkmn_name)
            or reserve_pkmn.base_name == pkmn_name
        ):
            return reserve_pkmn
    return None
...
def switch_or_drag(battle, split_msg):
    ...
    if side.active is not None:
        ...
        if (
            constants.TYPECHANGE in side.active.volatile_statuses
            and not side.active.terastallized
        ):
...
def move(battle, split_msg):
    ...
    try:
        if (
            constants.BOOSTS in all_move_json[move_name]
            and all_move_json[move_name][constants.CATEGORY] == constants.STATUS
        ):
            logger.debug(
                "{} used a boosting status-move. Setting can_have_choice_item to False".format(
                    pkmn.name
                )
            )
            pkmn.can_have_choice_item = False
    ...
    try:
        if all_move_json[move_name][constants.CATEGORY] == constants.STATUS:
            logger.debug(
                "{} used a status-move. Setting can_have_assultvest to False".format(
                    pkmn.name
                )
            )
...
def check_choicescarf(battle, msg_lines):
    ...
    if (
        len(moves) != 2
        or moves[0][0].startswith(battle.user.name)
        or moves[0][1][constants.PRIORITY] != moves[1][1][constants.PRIORITY]
    ):
        return
...
def check_choice_band_or_specs(battle, damage_dealt):
    ...
    try:
        ...
        logger.debug(
            "Could not find the move {}, skipping choice item check".format(move)
        )
        return
    ...
    potential_battles = battle.prepare_battles(
        guess_mega_evo_opponent=False, join_moves_together=True
    )
    ...
    if max_damage_without_choice_item == float(
        "-inf"
    ) or min_damage_with_choice_item == float("inf"):
        return
    ...
    if actual_damage_dealt > (
        max_damage_without_choice_item * 1.2
    ):  # multiply to avoid rounding errors
    ...
    if (
        actual_damage_dealt < (min_damage_with_choice_item * 0.8)
        ...
        logger.debug(
            "{} did not do enough damage to have {}".format(
...
def check_heavydutyboots(battle, msg_lines):
    ...
    if side_to_check.active.item != constants.UNKNOWN_ITEM or "magicguard" in [
        normalize_name(a)
