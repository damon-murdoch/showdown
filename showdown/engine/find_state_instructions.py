...
from .special_effects.abilities.modify_attack_against import (
    ability_modify_attack_against,
)
from .special_effects.abilities.modify_attack_being_used import (
    ability_modify_attack_being_used,
)
from .special_effects.items.modify_attack_against import item_modify_attack_against
from .special_effects.items.modify_attack_being_used import (
...
def lookup_move(move_name):
    if move_name.startswith(constants.SWITCH_STRING + " "):
        ...
        return {constants.SWITCH_STRING: split_move[1]}
...
def get_effective_speed(state, side):
    ...
    if (
        ...
        and side.active.ability == "surgesurfer"
    ):
        boosted_speed *= 2
    ...
    if any(
        vs in side.active.volatile_status
...
def get_effective_priority(side, move, field):
    ...
    if (
        side.active.ability == "prankster"
        and move[constants.CATEGORY] == constants.STATUS
    ):
        priority += 1
    elif (
        side.active.ability == "galewings"
        and (side.active.hp == side.active.maxhp)
        and ("flying" in move[constants.TYPE])
    ):
        priority += 1
...
def user_moves_first(state, user_move, opponent_move):
    ...
    if (
        constants.SWITCH_STRING in user_move
        and constants.SWITCH_STRING in opponent_move
    ):
    ...
    opponent_priority = get_effective_priority(
        state.opponent, opponent_move, state.field
    )
...
def update_attacking_move(
    ...
    attacking_move = item_modify_attack_being_used(
        attacking_pokemon.item, attacking_move, attacking_pokemon, defending_pokemon
    )

    attacking_move = ability_modify_attack_against(
        defending_pokemon.ability, attacking_move, attacking_pokemon, defending_pokemon
    )

    attacking_move = item_modify_attack_against(
        defending_pokemon.item, attacking_move, attacking_pokemon, defending_pokemon
    )

    if (
        constants.CHARGE in attacking_move[constants.FLAGS]
        and attacking_move[constants.ID] not in attacking_pokemon.volatile_status
    ):
    ...
    if (
        constants.PROTECT in attacking_move[constants.FLAGS]
        and any(
            vs in constants.PROTECT_VOLATILE_STATUSES
            for vs in defending_pokemon.volatile_status
        )
        and not (
            attacking_pokemon.ability == "unseenfist"
            and constants.CONTACT in attacking_move[constants.FLAGS]
        )
    ):
        attacking_move = attacking_move.copy()
        attacking_move[constants.ACCURACY] = False
        if (
            constants.BANEFUL_BUNKER in defending_pokemon.volatile_status
            and constants.CONTACT in attacking_move[constants.FLAGS]
        ):
            ...
            if constants.CRASH in attacking_move:
                ...
                attacking_move[constants.HEAL] = [
                    -1 * attacking_move[constants.CRASH][0],
                    attacking_move[constants.CRASH][1],
                ]
        elif (
            constants.SPIKY_SHIELD in defending_pokemon.volatile_status
            and constants.CONTACT in attacking_move[constants.FLAGS]
        ):
            attacking_move[constants.ACCURACY] = True
            attacking_move[constants.CATEGORY] = constants.STATUS
...
def cannot_use_move(attacking_pokemon, attacking_move):
    return (
        constants.TAUNT in attacking_pokemon.volatile_status
        and attacking_move[constants.CATEGORY] not in constants.DAMAGING_CATEGORIES
    )
...
def get_state_instructions_from_move(
    ...
    if constants.SWITCH_STRING in attacking_move:
        return [
            instruction_generator.get_instructions_from_switch(
                mutator, attacker, attacking_move[constants.SWITCH_STRING], instructions
            )
        ]
    ...
    if attacking_move[constants.CATEGORY] in constants.DAMAGING_CATEGORIES:
        ...
        if attacking_move_secondary:
            # flinching (iron head)
            if (
                attacking_move_secondary.get(constants.VOLATILE_STATUS)
                == constants.FLINCH
                and first_move
            ):
            ...
            # boosts from secondary, but to the defender (crunch)
                ...
                attacking_move_secondary
                and attacking_move_secondary.get(constants.BOOSTS) is not None
            ):
    ...
    all_instructions = (
        instruction_generator.get_instructions_from_statuses_that_freeze_the_state(
            mutator, attacker, defender, attacking_move, defending_move, instructions
        )
    )
    ...
    for instruction_set in all_instructions:
        temp_instructions += (
            instruction_generator.get_instructions_from_move_special_effect(
                mutator,
                attacker,
                attacking_pokemon,
                defending_pokemon,
    ...
    if damage_amounts is not None:
        ...
        for instruction_set in all_instructions:
            ...
            for dmg in damage_amounts:
                ...
                temp_instructions += instruction_generator.get_instructions_from_damage(
                    mutator,
                    defender,
                    dmg,
                    move_accuracy,
    ...
    if defending_pokemon.ability in constants.ABILITY_AFTER_MOVE:
        temp_instructions = []
        for instruction_set in all_instructions:
            temp_instructions += instruction_generator.get_instructions_from_defenders_ability_after_move(
                mutator,
                attacking_move,
    ...
    if side_condition is not None:
        ...
        for instruction_set in all_instructions:
            temp_instructions += (
                instruction_generator.get_instructions_from_side_conditions(
                    mutator, attacker, move_target, side_condition, instruction_set
                )
            )
        all_instructions = temp_instructions
    ...
    if volatile_status is not None:
        ...
        for instruction_set in all_instructions:
            temp_instructions += (
                instruction_generator.get_instructions_from_volatile_statuses(
                    mutator,
                    volatile_status,
                    attacker,
                    move_target,
    ...
    if move_status_effect is not None:
        ...
        for instruction_set in all_instructions:
            temp_instructions += (
                instruction_generator.get_instructions_from_status_effects(
                    mutator,
                    move_status_target,
                    move_status_effect,
                    move_status_accuracy,
                    instruction_set,
                )
            )
        all_instructions = temp_instructions
    ...
    if boosts is not None:
        temp_instructions = []
        for instruction_set in all_instructions:
            temp_instructions += instruction_generator.get_instructions_from_boosts(
    ...
    if attacking_move[constants.ID] in constants.BOOST_RESET_MOVES:
        ...
        for instruction_set in all_instructions:
            temp_instructions += (
                instruction_generator.get_instructions_from_boost_reset_moves(
                    mutator, attacking_move, attacker, instruction_set
                )
            )
        all_instructions = temp_instructions
    ...
    if flinch_accuracy is not None:
        ...
        for instruction_set in all_instructions:
            temp_instructions += (
                instruction_generator.get_instructions_from_flinching_moves(
    ...
    if (
        constants.DRAG in attacking_move[constants.FLAGS]
        and attacking_move[constants.ACCURACY]
    ):
        temp_instructions = []
        for instruction_set in all_instructions:
            temp_instructions += instruction_generator.get_instructions_from_drag(
                mutator, attacker, move_target, instruction_set
            )
        all_instructions = temp_instructions
    ...
    if switch_out_move_triggered(attacking_move, damage_amounts):
        ...
        for i in all_instructions:
            best_switch = get_best_switch_pokemon(
                mutator, i, attacker, attacking_side, defending_move, first_move
            )
...
def end_of_turn_triggered(user_move, opponent_move):
    if (
        user_move.startswith(constants.SWITCH_STRING + " ")
        and opponent_move == constants.DO_NOTHING_MOVE
    ):
        return False
    elif (
        opponent_move.startswith(constants.SWITCH_STRING + " ")
        and user_move == constants.DO_NOTHING_MOVE
    ):
        return False
...
def get_all_state_instructions(mutator, user_move_string, opponent_move_string):
    ...
    all_instructions = []
    if bot_moves_first:
        instructions = get_state_instructions_from_move(
            mutator,
            user_move,
            opponent_move,
            constants.USER,
        ...
        for instruction in instructions:
            all_instructions += get_state_instructions_from_move(
                mutator,
                opponent_move,
                user_move,
                constants.OPPONENT,
    ...
    if end_of_turn_triggered(user_move_string, opponent_move_string):
        temp_instructions = []
        for instruction_set in all_instructions:
            temp_instructions += instruction_generator.get_end_of_turn_instructions(
                mutator, instruction_set, user_move, opponent_move, bot_moves_first
            )
        all_instructions = temp_instructions
