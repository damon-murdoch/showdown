        ...
        if attacking_pokemon.hp:
            return (
                ...
                min(
                    ...
                    round(0.125 * attacking_pokemon.maxhp),
                ),
            )
...
def speedboost(
    ...
    if attacking_pokemon.speed_boost < 6:
        return (constants.MUTATOR_BOOST, attacking_side, constants.SPEED, 1)


def hydration(
    state, attacking_side, attacking_pokemon, defending_side, defending_pokemon
):
    if attacking_pokemon.status is not None and state.weather in [
        constants.RAIN,
...
def raindish(
    ...
    if state.weather == constants.RAIN or state.weather == constants.HEAVY_RAIN:
        return (
            ...
            min(
                attacking_pokemon.maxhp - attacking_pokemon.hp,
                round(0.0625 * attacking_pokemon.maxhp),
            ),
        )
...
def ability_end_of_turn(
    ...
    if (
        ...
        or not attacking_pokemon.hp
    ):
        return None
    try:
        return globals()[ability_name](
            state, attacking_side, attacking_pokemon, defending_side, defending_pokemon
        )
    except KeyError:
        pass
