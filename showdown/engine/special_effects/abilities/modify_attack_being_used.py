...
def adaptability(
    ...
    if attacking_move[constants.TYPE] in attacking_pokemon.types:
        attacking_move = attacking_move.copy()
        attacking_move[constants.BASE_POWER] = int(
            attacking_move[constants.BASE_POWER] * 4 / 3
        )
    return attacking_move
...
def contrary(
    ...
    if attacking_move[constants.TARGET] in constants.MOVE_TARGET_SELF:
        ...
        constants.SELF in attacking_move
        and constants.BOOSTS in attacking_move[constants.SELF]
    ):
        attacking_move = attacking_move.copy()
        ...
        attacking_move[constants.SECONDARY]
        and constants.SELF in attacking_move[constants.SECONDARY]
    ):
        if constants.BOOSTS in attacking_move[constants.SECONDARY][constants.SELF]:
            ...
            attacking_move[constants.SECONDARY][constants.SELF] = attacking_move[
                constants.SECONDARY
            ][constants.SELF].copy()
            attacking_move[constants.SECONDARY][constants.SELF][
                constants.BOOSTS
            ] = attacking_move[constants.SECONDARY][constants.SELF][
                constants.BOOSTS
            ].copy()
            for k, v in attacking_move[constants.SECONDARY][constants.SELF][
                constants.BOOSTS
            ].items():
...
def guts(
    ...
    if (
        attacking_pokemon.status is not None
...
def blaze(
    ...
    if (
        ...
        and (attacking_pokemon.hp / attacking_pokemon.maxhp) <= 1 / 3
    ):
        attacking_move = attacking_move.copy()
        attacking_move[constants.BASE_POWER] *= 1.5
    return attacking_move
...
def swarm(
    ...
    if (
        attacking_move[constants.TYPE] == "bug"
        and (attacking_pokemon.hp / attacking_pokemon.maxhp) <= 1 / 3
    ):
        attacking_move = attacking_move.copy()
        attacking_move[constants.BASE_POWER] *= 1.5
...
def sandforce(
    ...
    if weather == constants.SAND and attacking_move[constants.TYPE] in [
        "ground",
        "rock",
        "steel",
    ]:
...
def darkaura(
    ...
    if (
        ...
        and defending_pokemon.ability != "aurabreak"
    ):
        attacking_move = attacking_move.copy()
...
def prankster(
    ...
    if (
        "dark" in defending_pokemon.types
        and attacking_move[constants.CATEGORY] == constants.STATUS
    ):
        attacking_move = attacking_move.copy()
...
def ability_modify_attack_being_used(
    ability_name,
    attacking_move,
    defending_move,
    attacking_pokemon,
