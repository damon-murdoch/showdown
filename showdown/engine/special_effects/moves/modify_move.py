...
def suckerpunch(
    ...
    if (
        not first_move
        or defending_move.get(constants.CATEGORY) not in constants.DAMAGING_CATEGORIES
    ):
        attacking_move = attacking_move.copy()
...
def foulplay(
    ...
    attacking_move = attacking_move.copy()
    attacking_move[constants.BASE_POWER] *= (
...
def storedpower(
    ...
    multiplier = (
        attacking_pokemon.attack_boost
...
def psyshock(
    ...
    attacking_move = attacking_move.copy()
    attacking_move[constants.BASE_POWER] *= (
        defending_stats[constants.SPECIAL_DEFENSE] / defending_stats[constants.DEFENSE]
    )
    return attacking_move
...
def avalanche(
    ...
    if (
        first_move is False
        and defending_move.get(constants.CATEGORY) in constants.DAMAGING_CATEGORIES
    ):
        attacking_move = attacking_move.copy()
        attacking_move[constants.BASE_POWER] *= 2
    return attacking_move
...
def gyroball(
    ...
    attacking_move[constants.BASE_POWER] = min(
        150, (25 * defender_speed / attacker_speed) + 1
    )
    return attacking_move
...
def electroball(
    ...
    speed_ratio = (
        defending_pokemon.calculate_boosted_stats()[constants.SPEED]
        / attacking_pokemon.calculate_boosted_stats()[constants.SPEED]
    )
...
def focuspunch(
    ...
    if (
        first_move
        or defending_move.get(constants.CATEGORY) in constants.DAMAGING_CATEGORIES
    ):
        attacking_move = attacking_move.copy()
...
def bodypress(
    ...
    boosted_stats = attacking_pokemon.calculate_boosted_stats()
    attacking_move[constants.BASE_POWER] *= (
        boosted_stats[constants.DEFENSE] / boosted_stats[constants.ATTACK]
    )
    return attacking_move
...
def heavyslam(
    ...
    try:
        weight_ratio = (
            pokedex[defending_pokemon.id][constants.WEIGHT]
            / pokedex[attacking_pokemon.id][constants.WEIGHT]
        )
