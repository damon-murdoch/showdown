...
def thickclub(attacking_move, attacking_pokemon, defending_pokemon):
    if (
        attacking_pokemon.id in ["cubone", "marowak", "marowakalola"]
...
def whiteherb(attacking_move, attacking_pokemon, defending_pokemon):
    if (
        constants.BOOSTS in attacking_move
        and attacking_move[constants.TARGET] in constants.MOVE_TARGET_SELF
    ):
        attacking_move = attacking_move.copy()
...
def souldew(attacking_move, attacking_pokemon, defending_pokemon):
    if (attacking_pokemon.id == "latios" or attacking_pokemon.id == "latias") and (
        attacking_move[constants.TYPE] == "psychic"
        or attacking_move[constants.TYPE] == "dragon"
    ):
        attacking_move = attacking_move.copy()
        attacking_move[constants.BASE_POWER] *= 1.2
    return attacking_move
...
def adamantorb(attacking_move, attacking_pokemon, defending_pokemon):
    if attacking_pokemon.id == "dialga" and (
        attacking_move[constants.TYPE] == "dragon"
        or attacking_move[constants.TYPE] == "steel"
    ):
        attacking_move = attacking_move.copy()
        attacking_move[constants.BASE_POWER] *= 1.2
    return attacking_move
...
def lustrousorb(attacking_move, attacking_pokemon, defending_pokemon):
    if attacking_pokemon.id == "palkia" and (
        attacking_move[constants.TYPE] == "dragon"
        or attacking_move[constants.TYPE] == "water"
    ):
        attacking_move = attacking_move.copy()
        attacking_move[constants.BASE_POWER] *= 1.2
    return attacking_move
...
def griseousorb(attacking_move, attacking_pokemon, defending_pokemon):
    if (
        attacking_pokemon.id == "giratina" or attacking_pokemon.id == "giratinaorigin"
    ) and (
        attacking_move[constants.TYPE] == "dragon"
        or attacking_move[constants.TYPE] == "ghost"
    ):
        attacking_move = attacking_move.copy()
        attacking_move[constants.BASE_POWER] *= 1.2
    return attacking_move
