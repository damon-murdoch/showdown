...
def mistyseed(
    ...
    if (
        state.field == constants.MISTY_TERRAIN
        and attacking_pokemon.special_defense_boost < 6
    ):
        return [
...
def psychicseed(
    ...
    if (
        state.field == constants.PSYCHIC_TERRAIN
        and attacking_pokemon.special_defense_boost < 6
    ):
        return [
            (constants.MUTATOR_BOOST, attacking_side, constants.SPECIAL_DEFENSE, 1),
...
def electricseed(
    ...
    if (
        ...
        and attacking_pokemon.defense_boost < 6
    ):
        return [
            (constants.MUTATOR_BOOST, attacking_side, constants.DEFENSE, 1),
            (
    ...
    if attacking_pokemon.ability in ["quarkdrive", "protosynthesis"]:
        ...
        return [
            (
                ...
                + constants.STAT_ABBREVIATION_REVERSE_LOOKUPS[highest_stat],
            ),
            (
                constants.MUTATOR_CHANGE_ITEM,
                attacking_side,
