...
def flameorb(
    ...
    if attacking_pokemon.status is None and "fire" not in attacking_pokemon.types:
        return (constants.MUTATOR_APPLY_STATUS, attacking_side, constants.BURN)
...
def toxicorb(
    ...
    if attacking_pokemon.status is None and "poison" not in attacking_pokemon.types:
        return (constants.MUTATOR_APPLY_STATUS, attacking_side, constants.TOXIC)


def item_end_of_turn(
    item_name,
    state,
    attacking_side,
    attacking_pokemon,
